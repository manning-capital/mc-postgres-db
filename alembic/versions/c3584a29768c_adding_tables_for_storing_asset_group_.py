...
def upgrade() -> None:
    ...
    op.create_table('provider_asset_group',
    ...
    sa.Column('provider_asset_group_id', sa.Integer(), nullable=False, comment='The identifier of the provider asset group'),
    ...
    sa.Column('linear_fit_alpha', sa.Float(), nullable=True, comment='The alpha parameter (intercept) for the linear fit equation to_asset_2 = alpha + beta * to_asset_1. The numbers correspond to the order in the asset group member table (to_asset_1 = to_asset_id with order of 1, to_asset_2 = to_asset_id with order of 2). From RollingOLS.params[-1, 0] when using sm.add_constant()'),
    sa.Column('linear_fit_beta', sa.Float(), nullable=True, comment='The beta parameter (slope) for the linear fit equation to_asset_2 = alpha + beta * to_asset_1. The numbers correspond to the order in the asset group member table (to_asset_1 = to_asset_id with order of 1, to_asset_2 = to_asset_id with order of 2). From RollingOLS.params[-1, 1] when using sm.add_constant()'),
    sa.Column('linear_fit_mse', sa.Float(), nullable=True, comment='The mean squared error (MSE) of the linear fit between to_asset_1 (independent variable) and to_asset_2 (dependent variable) in the asset group pair. The numbers correspond to the order in the asset group member table (to_asset_1 = to_asset_id with order of 1, to_asset_2 = to_asset_id with order of 2). From RollingOLS.mse_resid'),
    sa.Column('linear_fit_r_squared', sa.Float(), nullable=True, comment='The R-squared (coefficient of determination) of the linear fit, indicating the proportion of variance explained by the regression. From RollingOLS.rsquared'),
    sa.Column('linear_fit_r_squared_adj', sa.Float(), nullable=True, comment='The adjusted R-squared of the linear fit, accounting for the number of predictors. From RollingOLS.rsquared_adj'),
    sa.ForeignKeyConstraint(['provider_asset_group_id'], ['provider_asset_group.id'], ),
    sa.PrimaryKeyConstraint('timestamp', 'provider_asset_group_id', 'lookback_window_seconds', name=op.f('provider_asset_group_attribute_pkey')),
    ...
    op.create_table('provider_asset_group_member',
    sa.Column('provider_asset_group_id', sa.Integer(), nullable=False, comment='The identifier of the provider asset group'),
    ...
    sa.Column('order', sa.Integer(), nullable=True, comment='The order of the asset pair within the group (1, 2, 3, etc.). If null, no specific order is assigned.'),
    ...
    sa.PrimaryKeyConstraint('provider_asset_group_id', 'provider_id', 'from_asset_id', 'to_asset_id', name=op.f('provider_asset_group_member_pkey')),
